...
public class UnicodeReader extends Reader {
	 ...
	 *             access to the file.
	 */
	public UnicodeReader(final File file) throws IOException {
	 ...
	 *             access to the file.
	 */
	public UnicodeReader(final File file, final String defaultEncoding) throws IOException {
	 ...
	 *            The input stream from which to read.
	 * @param defaultEncoding
	 *            The encoding to use if no recognized BOM is found. If this value
	 ...
	 *             access to the file.
	 */
	public UnicodeReader(final String file) throws IOException {
		this(new File(file));
	}
	...
	protected void init(final InputStream in, final String defaultEncoding) throws IOException {
		...
		final PushbackInputStream tempIn = new PushbackInputStream(in, UnicodeReader.BOM_SIZE);

		final byte[] bom = new byte[UnicodeReader.BOM_SIZE];
		int n, unread;
		...
		else if (n == UnicodeReader.BOM_SIZE && // Last 2 bytes are 0; could be an empty UTF-16
		...
		if (this.encoding == null) {
			...
			this.encoding = this.internalIn.getEncoding(); // Get the default.
		} else
