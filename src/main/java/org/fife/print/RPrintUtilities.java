...
public abstract class RPrintUtilities {
	...
	private static class RPrintTabExpander implements TabExpander {
		...
		public float nextTabStop(final float x, final int tabOffset) {
			...
			return RPrintUtilities.xOffset + (ntabs + 1f) * tabSizeInPixels;
		}
	 ...
	 *            Only up-to this many characters from <code>line</code> can be
	 *            printed on one line.
	...
	private static int getLineBreakPoint(final String line, final int maxCharsPerLine) {
		...
		int breakPoint = -1;
		for (final char element : RPrintUtilities.BREAK_CHARS) {
			final int breakCharPos = line.lastIndexOf(element, maxCharsPerLine - 1);
	 ...
	 */
	public static int printDocumentMonospaced(final Graphics g, final Document doc, final int fontSize,
			final int pageIndex, final PageFormat pageFormat, final int tabSize) {
		...
		final int fontWidth = RPrintUtilities.fm.charWidth('w'); // Any character will do as font is monospaced.
		...
		RPrintUtilities.currentDocLineNumber = 0; // The line number of the document we're currently on.
		RPrintUtilities.rootElement = doc.getDefaultRootElement(); // To shorten accesses in our loop.
		RPrintUtilities.numDocLines = RPrintUtilities.rootElement.getElementCount(); // The number of lines in our
																						// document.
		while (RPrintUtilities.currentDocLineNumber < RPrintUtilities.numDocLines) {
			...
			if (RPrintUtilities.tabSizeInSpaces == 0)
				...
				while (tabIndex > -1) {
					final int spacesNeeded = RPrintUtilities.tabSizeInSpaces
							- tabIndex % RPrintUtilities.tabSizeInSpaces;
			...
			RPrintUtilities.currentDocLineNumber += 1; // We have printed one more line from the document.
	 ...
	 */
	public static int printDocumentMonospacedWordWrap(final Graphics g, final Document doc, final int fontSize,
			final int pageIndex, final PageFormat pageFormat, final int tabSize) {
		...
		// Get width and height of characters in this monospaced font.
		final int fontWidth = RPrintUtilities.fm.charWidth('w'); // Any character will do here, since font is
																	// monospaced.
		final int fontHeight = RPrintUtilities.fm.getHeight();
		...
		while (RPrintUtilities.currentDocLineNumber < RPrintUtilities.numDocLines) {
			...
			while (curLineString.length() > maxCharsPerLine) {
				...
				final int breakPoint = RPrintUtilities.getLineBreakPoint(curLineString, maxCharsPerLine) + 1;
	 ...
	 *            The graphics context to write to.
	 * @param textComponent
	 *            The <code>javax.swing.text.JTextComponent</code> whose text you're
	 *            printing.
	 * @param font
	 ...
	 *
	 */
	public static int printDocumentWordWrap(final Graphics g, final JTextComponent textComponent, final Font font,
			final int pageIndex, final PageFormat pageFormat, final int tabSize) {
		...
		while (RPrintUtilities.currentDocLineNumber < RPrintUtilities.numDocLines) {
			...
			// Get the current line (as an Element), and its starting and ending offset in
			// doc.
			final Element currentLine = RPrintUtilities.rootElement.getElement(RPrintUtilities.currentDocLineNumber);
			...
			int currentLineLengthInPixels = Utilities.getTabbedTextWidth(currentLineSeg, RPrintUtilities.fm, 0,
					tabExpander, 0);
			...
			if (currentLineLengthInPixels <= lineLengthInPixels) {
				RPrintUtilities.currentDocLineNumber += 1; // We (will) have printed one more line from the document.
			...
			// If it won't fit on a printed line by itself (i.e., it needs to be wrapped)...
				...
				while (currentLineLengthInPixels > lineLengthInPixels) {
					...
					for (final char element : RPrintUtilities.BREAK_CHARS) {
						...
						final int pos = currentLineString.lastIndexOf(element) + 1;
					...
					if (currentPos == -1) {
						...
						do {
							...
							currentLineLengthInPixels = Utilities.getTabbedTextWidth(currentLineSeg, RPrintUtilities.fm,
									0, tabExpander, 0);
						} while (currentLineLengthInPixels <= lineLengthInPixels);
						currentPos--;
					...
					currentLineLengthInPixels = Utilities.getTabbedTextWidth(currentLineSeg, RPrintUtilities.fm, 0,
							tabExpander, 0);
