...
public abstract class RTextAreaBase extends JTextArea {
	...
	public static final int getDefaultMarginLinePosition() {
		return RTextAreaBase.DEFAULT_MARGIN_LINE_POSITION;
	}
	...
	public static final int getDefaultTabSize() {
		return RTextAreaBase.DEFAULT_TAB_SIZE;
	}
	...
	private void addCurrentLineHighlightListeners() {
		...
		for (final MouseMotionListener mouseMotionListener2 : mouseMotionListeners)
			if (mouseMotionListener2 == this.mouseListener) {
				add = false;
				break;
			}
		if (add)
		...
		for (final MouseListener mouseListener2 : mouseListeners)
			if (mouseListener2 == this.mouseListener) {
				add = false;
				break;
			}
		if (add)
	...
	public void addNotify() {
		...
		if (this.getCaretPosition() != 0)
			SwingUtilities.invokeLater(() -> RTextAreaBase.this.possiblyUpdateCurrentLineHighlightLocation());
	}
	...
	public final Object getBackgroundObject() {
		if (this.backgroundPainter == null)
			return null;
		return this.backgroundPainter instanceof ImageBackgroundPainterStrategy
				? (Object) ((ImageBackgroundPainterStrategy) this.backgroundPainter).getMasterImage()
				: (Object) ((ColorBackgroundPainterStrategy) this.backgroundPainter).getColor();
	}
	...
	protected void init() {
		...
		this.setWrapStyleWord(true);// We only support wrapping at word boundaries.
		this.setTabSize(5);
	...
	protected void processComponentEvent(final ComponentEvent e) {
		...
		if (e.getID() == ComponentEvent.COMPONENT_RESIZED && this.getLineWrap() && this.getHighlightCurrentLine()) {
			this.previousCaretY = -1; // So we are sure to repaint.
			this.fireCaretUpdate(this.mouseListener);
		}
	...
	public void setBackground(final Color bg) {
		...
		this.setOpaque(bg == null || bg.getAlpha() == 0xff);
	...
	public void setBackgroundImage(final Image image) {
		...
		this.firePropertyChange(RTextAreaBase.BACKGROUND_IMAGE_PROPERTY, oldBG, image);
		this.repaint();
	}
	...
	public void setCurrentLineHighlightColor(final Color color) {
		...
		if (!color.equals(this.currentLineColor)) {
			...
			this.firePropertyChange(RTextAreaBase.CURRENT_LINE_HIGHLIGHT_COLOR_PROPERTY, old, color);
		}
	}
	...
	public void setFadeCurrentLineHighlight(final boolean fade) {
		if (fade != this.fadeCurrentLineHighlight) {
			...
			this.firePropertyChange(RTextAreaBase.CURRENT_LINE_HIGHLIGHT_FADE_PROPERTY, !fade, fade);
		}
	}
	...
	public void setHighlightCurrentLine(final boolean highlight) {
		if (highlight != this.highlightCurrentLine) {
			...
			this.firePropertyChange(RTextAreaBase.HIGHLIGHT_CURRENT_LINE_PROPERTY, !highlight, highlight);
			this.repaint(); // Repaint entire width of line.
		}
	}
	...
	public void setRoundedSelectionEdges(final boolean rounded) {
		if (this.roundedSelectionEdges != rounded) {
			...
			this.firePropertyChange(RTextAreaBase.ROUNDED_SELECTION_PROPERTY, !rounded, rounded);
		}
	}
