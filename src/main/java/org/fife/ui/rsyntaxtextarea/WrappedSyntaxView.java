...
public class WrappedSyntaxView extends BoxView implements TabExpander, RSTAView {
	...
	class WrappedLine extends View {
		...
		final int calculateLineCount() {
			...
			for (int p0 = startOffset; p0 < p1;) {
				...
				nlines += 1;
				final TokenSubList subList = TokenUtils.getSubTokenList(tokenList, p0, WrappedSyntaxView.this, textArea,
						x0, WrappedSyntaxView.this.lineCountTempToken);
		...
		public Shape modelToView(final int pos, final Shape a, final Position.Bias b) throws BadLocationException {
			...
			while (p0 < p1) {
				...
				x0 = subList != null ? subList.x : x0;
				tokenList = subList != null ? subList.tokenList : null;
				final int p = WrappedSyntaxView.this.calculateBreakPosition(p0, tokenList, x0);
				if (pos >= p0 && testP < p) {// pos < p)) {
					// it's in this line
					alloc = RSyntaxUtilities.getLineWidthUpTo(textArea, WrappedSyntaxView.this.s, p0, pos,
		...
		public int viewToModel(final float fx, final float fy, final Shape a, final Position.Bias[] bias) {
			...
			if (y < alloc.y)
				...
				while (p0 < p1) {
					...
					// line.
					final TokenSubList subList = TokenUtils.getSubTokenList(tlist, p0, WrappedSyntaxView.this, textArea,
							alloc.x, WrappedSyntaxView.this.lineCountTempToken);
					tlist = subList != null ? subList.tokenList : null;
					final int p = WrappedSyntaxView.this.calculateBreakPosition(p0, tlist, alloc.x);
	...
	private final TokenImpl lineCountTempToken;
	 ...
	 */
	private final Segment s, drawSeg;
	private int tabBase;
	private int tabSize;
	/**
	...
	protected int calculateBreakPosition(final int p0, final Token tokenList, float x0) {
		...
		if (currentWidth == Integer.MAX_VALUE)
			currentWidth = this.getPreferredSpan(View.X_AXIS);
		...
		currentWidth = Math.max(currentWidth, WrappedSyntaxView.MIN_WIDTH);
		Token t = tokenList;
	...
	private void childAllocation2(final int line, final int y, final Rectangle alloc) {
		alloc.x += this.getOffset(View.X_AXIS, line);
		alloc.y += y;
		alloc.width = this.getSpan(View.X_AXIS, line);
		alloc.height = this.getSpan(View.Y_AXIS, line);
	 ...
	 */
	protected void drawView(final TokenPainter painter, final Graphics2D g, final Rectangle r, final View view,
			final int fontHeight, int y, final int line) {
		...
		final int p1 = view.getEndOffset();// - 1;
		...
		while (token != null && token.isPaintable()) {
			...
			final int p = this.calculateBreakPosition(p0, token, x);
			x = r.x;
			...
			while (token != null && token.isPaintable() && token.getEndOffset() - 1 < p) {// <=p) {
				...
				x = painter.paint(token, g, x, y, this.host, this, 0, paintBG);
				token = token.getNextToken();
			}

			if (token != null && token.isPaintable() && token.getOffset() < p) {
				final int tokenOffset = token.getOffset();
				this.tempToken.set(this.drawSeg.array, tokenOffset - start, p - 1 - start, tokenOffset,
						token.getType());
				this.tempToken.setLanguageIndex(token.getLanguageIndex());
				final boolean paintBG = this.host.getPaintTokenBackgrounds(line, y);
				painter.paint(this.tempToken, g, x, y, this.host, this, 0, paintBG);
	 ...
	 */
	protected void drawViewWithSelection(final TokenPainter painter, final Graphics2D g, final Rectangle r,
			final View view, final int fontHeight, int y, final int selStart, final int selEnd) {
		...
		while (token != null && token.isPaintable()) {
			...
			if (token != null && token.isPaintable() && token.getOffset() < p) {
				...
				token = new TokenImpl(this.drawSeg, tokenOffset - start, p - 1 - start, tokenOffset, token.getType(),
	...
	public Shape getChildAllocation(final int index, final Shape a) {
		if (a != null) {
			...
			if (ca != null && !this.isAllocationValid()) {
				...
				final Rectangle r = ca instanceof Rectangle ? (Rectangle) ca : ca.getBounds();
	...
	public float getMaximumSpan(final int axis) {
		...
		if (axis == View.X_AXIS)
			span += this.metrics.charWidth('\u00b6'); // metrics set in updateMetrics
		return span;
	}
			...
			if (this.host.isCodeFoldingEnabled()) {
				...
				for (int i = 0; i < lineCount; i++)
					if (fm.isLineHidden(i))
						span -= this.getSpan(View.Y_AXIS, i);
			}
		}
		return span;
	}
	...
	protected View getViewAtPoint(final int x, final int y, final Rectangle alloc) {
		...
		int curY = alloc.y + this.getOffset(View.Y_AXIS, 0); // Always at least 1 line
		this.host = (RSyntaxTextArea) this.getContainer();
		final FoldManager fm = this.host.getFoldManager();

		for (int line = 1; line < lineCount; line++) {
			final int span = this.getSpan(View.Y_AXIS, line - 1);
			if (y < curY + span) {
	...
	public void paint(final Graphics g, final Shape a) {
		...
		final int n = this.getViewCount(); // Number of lines.
		final int x = alloc.x + this.getLeftInset();
		this.tempRect.y = alloc.y + this.getTopInset();
		final Rectangle clip = g.getClipBounds();
		for (int i = 0; i < n; i++) {

			this.tempRect.x = x + this.getOffset(View.X_AXIS, i);
			// tempRect.y = y + getOffset(Y_AXIS, i);
			this.tempRect.width = this.getSpan(View.X_AXIS, i);
			this.tempRect.height = this.getSpan(View.Y_AXIS, i);
			...
			if (this.tempRect.intersects(clip)) {
				...
				if (selStart == selEnd || startOffset >= selEnd || endOffset < selStart)
					...
					this.drawViewWithSelection(painter, g2d, alloc, view, fontHeight, this.tempRect.y + ascent,
							selStart, selEnd);
			}
	...
	final void updateMetrics() {
		...
		this.metrics = host.getFontMetrics(f); // Metrics for the default font.
	...
	public int viewToModel(final float x, final float y, final Shape a, final Position.Bias[] bias) {
		...
		if (v != null) {
			...
			if (this.host.isCodeFoldingEnabled() && v == this.getView(this.getViewCount() - 1)
	...
	public int yForLineContaining(final Rectangle alloc, final int offs) throws BadLocationException {
		if (this.isAllocationValid()) {
			...
			final Rectangle r = (Rectangle) this.modelToView(offs, alloc, Bias.Forward);
			if (r != null) {
				if (this.host.isCodeFoldingEnabled()) {
