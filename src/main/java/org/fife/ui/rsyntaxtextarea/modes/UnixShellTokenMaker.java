...
public class UnixShellTokenMaker extends AbstractTokenMaker {
	...
	// identifiers.
	protected final String shellVariables = "#-?$!*@_"; // Characters that are part of "$<char>" shell variables; e.g.,
														// "$_".
	...
	public Token getTokenList(final Segment text, final int startTokenType, final int startOffset) {
		...
		for (int i = offset; i < end; i++) {
			...
			case TokenTypes.NULL:
				...
				case '`':
					if (backslash) { // Escaped back quote => call '`' an identifier..
						...
						this.currentTokenType = TokenTypes.LITERAL_BACKQUOTE;
					break;
				...
				case '"':
					if (backslash) { // Escaped double quote => call '"' an identifier..
						...
						this.currentTokenType = TokenTypes.LITERAL_STRING_DOUBLE_QUOTE;
					break;
				...
				case '\'':
					if (backslash) { // Escaped single quote => call '\'' an identifier.
						...
						this.currentTokenType = TokenTypes.LITERAL_CHAR;
					break;
				...
				default:
					if (RSyntaxUtilities.isDigit(c)) {
						this.currentTokenType = TokenTypes.LITERAL_NUMBER_DECIMAL_INT;
						break;
			...
			case TokenTypes.LITERAL_NUMBER_DECIMAL_INT:
				...
				case ' ':
				case '\t':
					this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.LITERAL_NUMBER_DECIMAL_INT,
							newStartOffset + this.currentTokenStart);
					this.currentTokenStart = i;
					this.currentTokenType = TokenTypes.WHITESPACE;
					break;
			...
			case TokenTypes.VARIABLE:
				...
				if (c == '{') {
					...
					if (i == end) { // Happens when '}' wasn't found...
						this.addToken(text, this.currentTokenStart, end - 1, TokenTypes.VARIABLE,
								newStartOffset + this.currentTokenStart);
						this.currentTokenType = TokenTypes.NULL;
					}
					break;
				...
				while (i < end) {
					...
					if (!RSyntaxUtilities.isLetterOrDigit(c) && this.shellVariables.indexOf(c) == -1 && c != '_') {
						this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.VARIABLE,
								newStartOffset + this.currentTokenStart);
						i--;
						this.currentTokenType = TokenTypes.NULL;
				...
				// This only happens if we never found the end of the variable in the loop
				// above.
				if (i == end) {
					this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.VARIABLE,
							newStartOffset + this.currentTokenStart);
			...
			case TokenTypes.COMMENT_EOL:
				...
				i = end - 1;
				this.addToken(text, this.currentTokenStart, i, this.currentTokenType,
						newStartOffset + this.currentTokenStart);
			...
			case TokenTypes.LITERAL_CHAR:
				...
				if (c == '\\')
					...
					if (c == '\'' && !backslash) {
						this.addToken(text, this.currentTokenStart, i, TokenTypes.LITERAL_CHAR,
								newStartOffset + this.currentTokenStart);
						this.currentTokenStart = i + 1;
						this.currentTokenType = TokenTypes.NULL;
			...
			case TokenTypes.LITERAL_BACKQUOTE:
				...
				case '`':
					if (!backslash) {
						this.addToken(text, this.currentTokenStart, i, TokenTypes.LITERAL_BACKQUOTE,
								newStartOffset + this.currentTokenStart);
						this.currentTokenType = TokenTypes.NULL;
						// backslash is definitely false when we leave.
						break;
				...
				case '$':
					...
					this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.LITERAL_BACKQUOTE,
							newStartOffset + this.currentTokenStart);
					this.currentTokenType = TokenTypes.VARIABLE;
					this.currentTokenStart = i;
							...
							if (array[i] == '}') {
								...
								if (i < end) {
									...
									if (c == '`') { // The only rub - back quote right after variable.
										this.addToken(text, i, i, TokenTypes.LITERAL_BACKQUOTE, newStartOffset + i);
										this.currentTokenType = TokenTypes.NULL;
										break;
			...
			case TokenTypes.LITERAL_STRING_DOUBLE_QUOTE:
				...
				case '$':
					...
					// Add the string up-to the variable.
					this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.LITERAL_STRING_DOUBLE_QUOTE,
							newStartOffset + this.currentTokenStart);
					this.currentTokenType = TokenTypes.VARIABLE;
					this.currentTokenStart = i;
							...
							if (array[i] == '}') {
								...
								if (i < end) {
									...
									if (c == '"') { // The only rub - double-quote right after variable.
										this.addToken(text, i, i, TokenTypes.LITERAL_STRING_DOUBLE_QUOTE,
												newStartOffset + i);
										this.currentTokenType = TokenTypes.NULL;
										break;
									} else { // Continue on with the string.
					...
					while (++i < end) {
						...
						if (!RSyntaxUtilities.isLetterOrDigit(c) && this.shellVariables.indexOf(c) == -1 && c != '_') {
							this.addToken(text, this.currentTokenStart, i - 1, TokenTypes.VARIABLE,
									newStartOffset + this.currentTokenStart);
							if (c == '"') { // The only rub.
								this.addToken(text, i, i, TokenTypes.LITERAL_STRING_DOUBLE_QUOTE, newStartOffset + i);
								this.currentTokenType = TokenTypes.NULL;
								break;
							} else {
								this.currentTokenStart = i;
	...
	public TokenMap getWordsToHighlight() {
		...
		final int function = TokenTypes.FUNCTION;
