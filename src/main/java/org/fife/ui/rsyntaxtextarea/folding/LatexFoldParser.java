...
public class LatexFoldParser implements FoldParser {
	...
	public List<Fold> getFolds(final RSyntaxTextArea textArea) {
		...
		final Stack<String> expectedStack = new Stack<>();
		...
		try {
			...
			for (int line = 0; line < lineCount; line++) {
				...
				while (t != null && t.isPaintable()) {
					...
					if (t.is(TokenTypes.RESERVED_WORD, LatexFoldParser.BEGIN)) {
					...
					else if (t.is(TokenTypes.RESERVED_WORD, LatexFoldParser.END) && currentFold != null
