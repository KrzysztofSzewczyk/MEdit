...
public class CurlyFoldParser implements FoldParser {
	 ...
	 *            Whether to scan for C-style multi-line comments and make them
	 *            foldable.
	 * @param java
	 *            Whether this parser is folding Java. This adds extra parsing
	 *            rules, such as grouping all import statements into a fold section.
	 */
			...
			for (int line = 0; line < lineCount; line++) {
				...
				while (t != null && t.isPaintable()) {
					...
					if (this.getFoldableMultiLineComments() && t.isComment()) {
						...
						if (t.getType() != TokenTypes.COMMENT_EOL && !t.endsWith(CurlyFoldParser.C_MLC_END)) {
					...
					// Java-specific folding rules
						...
						if (t.is(TokenTypes.RESERVED_WORD, CurlyFoldParser.KEYWORD_IMPORT)) {
							if (importStartLine == -1) {
